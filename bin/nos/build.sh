...
# --- Bassie Android Build Script v1.1 (modified) ---
...
if [ "$1" == "key" ]; then
    ...
    if aapt2 compile --dir res -o res-compiled; then
        if aapt2 link res-compiled/*.flat --manifest AndroidManifest.xml --java src -I $PLATFORM -o $name-unaligned.apk; then
            ...
            if javac -Xlint -cp "$PLATFORM:src-compiled" -d src-compiled @sources.txt; then
            ...
            rm -f -r src-compiled/nl sources.txt
        fi
