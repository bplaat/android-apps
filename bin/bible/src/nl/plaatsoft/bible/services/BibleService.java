...
public class BibleService {
    ...
    public ArrayList<Bible> getInstalledBibles(Context context) {
        ...
        Collections.sort(bibles,
            (a, b)
                -> Integer.compare(bibleLanguageOrder.indexOf(a.language()), bibleLanguageOrder.indexOf(b.language())));
        return bibles;
    }
        ...
        if (readIndex) {
            ...
            try (var testamentsCursor = database.rawQuery("SELECT id, key, name FROM testaments", null)) {
                while (testamentsCursor.moveToNext()) {
                    ...
                    try (var booksCursor = database.rawQuery("SELECT id, key, name FROM books WHERE testament_id = ?",
                        ...
                        while (booksCursor.moveToNext()) {
                            ...
                            try (var chaptersCursor =
                                     database.rawQuery("SELECT id, number FROM chapters WHERE book_id = ?",
                                         new String[] {String.valueOf(bookId)})) {
                                while (chaptersCursor.moveToNext()) {
                                    chapters.add(new Chapter(chaptersCursor.getInt(chaptersCursor.getColumnIndex("id")),
                                        chaptersCursor.getInt(chaptersCursor.getColumnIndex("number"))));
                                }
                            }

                            books.add(new Book(bookId, booksCursor.getString(booksCursor.getColumnIndex("key")),
                                booksCursor.getString(booksCursor.getColumnIndex("name")), chapters));
                        }
                    }

                    testaments.add(
                        new Testament(testamentId, testamentsCursor.getString(testamentsCursor.getColumnIndex("key")),
                            testamentsCursor.getString(testamentsCursor.getColumnIndex("name")), books));
                }
            }
        }
    ...
    public @Nullable ChapterWithVerses readChapter(Context context, String path, String bookKey, int chapterNumber) {
        ...
        try (var chapterCursor = database.rawQuery(
            ...
            try (var versesCursor =
                     database.rawQuery("SELECT id, number, text, is_subtitle, is_last FROM verses WHERE chapter_id = ?",
                         new String[] {String.valueOf(chapterId)})) {
                while (versesCursor.moveToNext()) {
                    verses.add(new Verse(versesCursor.getInt(versesCursor.getColumnIndex("id")),
            ...
            return new ChapterWithVerses(
                chapterId, chapterCursor.getInt(chapterCursor.getColumnIndex("number")), verses);
        }
    }
    ...
    public ArrayList<SearchVerse> searchVerses(Context context, String path, String query, int maxResults) {
        ...
        try (var cursor =
                 database.rawQuery("SELECT books.id AS book_id, books.key AS book_key, books.name AS book_name, "
            ...
            while (cursor.moveToNext()) {
                results.add(new SearchVerse(new Verse(cursor.getInt(cursor.getColumnIndex("verse_id")),
                    ...
                    new Book(cursor.getInt(cursor.getColumnIndex("book_id")),
                        cursor.getString(cursor.getColumnIndex("book_key")),
                        cursor.getString(cursor.getColumnIndex("book_name")), new ArrayList<>()),
                    new Chapter(cursor.getInt(cursor.getColumnIndex("chapter_id")),
